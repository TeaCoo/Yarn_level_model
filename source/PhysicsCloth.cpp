...
void PhysicsCloth::FillMV(int type = 1) 
{
	/*
	clock_t Thread1Time = 0, Thread1StartTime = 9999999999, thread1cost = 0, thread1end = 0;
	clock_t Thread2Time = 0, Thread2StartTime = 9999999999, thread2cost = 0, thread2end = 0;
	clock_t Thread3Time = 0, Thread3StartTime = 9999999999, thread3cost = 0, thread3end = 0;
	clock_t Thread4Time = 0, Thread4StartTime = 9999999999, thread4cost = 0, thread4end = 0;
	clock_t Thread5Time = 0, Thread5StartTime = 9999999999, thread5cost = 0, thread5end = 0;
	clock_t Thread6Time = 0, Thread6StartTime = 9999999999, thread6cost = 0, thread6end = 0;
	clock_t Thread7Time = 0, Thread7StartTime = 9999999999, thread7cost = 0, thread7end = 0;
	clock_t Thread11Time = 0;
	clock_t Thread21Time = 0;
	clock_t Thread31Time = 0;
	clock_t Thread41Time = 0;
	clock_t Thread51Time = 0;
	clock_t Thread61Time = 0;
	clock_t Thread71Time = 0;
	...
	*/
	...
	//std::cout << "Thread start time: " << startT << " ms;" << std::endl;
#pragma omp parallel
	{
...
#pragma omp for collapse(2) nowait
		for (int i = 0; i < env->cloth->width; ++i)
			for (int j = 0; j < env->cloth->length; ++j) {
				...
				//if (threadStart < Thread1StartTime)
					//Thread1StartTime = threadStart;
				int ID = omp_get_thread_num();
				Constraint_F(i, j, F_Ls[ID]);
				Constraint_D(i, j, F_LL_Poss[ID]);
				/*
				thread1end = clock();
				if (thread1end - startT > Thread1Time)
					Thread1Time = thread1end - startT;
				if (thread1end - threadStart > Thread11Time)
					Thread11Time = thread1end - threadStart;
				if (thread1end - Thread1StartTime > thread1cost)
					thread1cost = thread1end - Thread1StartTime;
				*/
			}
...
#pragma omp for nowait
		for (int i = 0; i < env->cloth->edges.size(); ++i) {
			//clock_t threadStart = clock();
			//if (threadStart < Thread2StartTime)
				//Thread2StartTime = threadStart;
			int ID = omp_get_thread_num();
			...
			/*
			thread2end = clock();
			if (thread2end - startT > Thread2Time)
				Thread2Time = thread2end - startT;
			if (thread2end - threadStart > Thread21Time)
				Thread21Time = thread2end - threadStart;
			if (thread2end - Thread2StartTime > thread2cost)
				thread2cost = thread2end - Thread2StartTime;
			*/
		}
...
#pragma omp for nowait
		for (int i = 0; i < env->cloth->bend_segs.size(); ++i) {
			/*
			clock_t threadStart = clock();
			if (threadStart < Thread3StartTime)
				Thread3StartTime = threadStart;
			*/
			int ID = omp_get_thread_num();
			...
			/*
			thread3end = clock();
			if (thread3end - startT > Thread3Time)
				Thread3Time = thread3end - startT;
			if (thread3end - threadStart > Thread31Time)
				Thread31Time = thread3end - threadStart;
			if (thread3end - Thread3StartTime > thread3cost)
				thread3cost = thread3end - Thread3StartTime;
			*/
		}
...
#pragma omp for
		for (int i = 0; i < env->cloth->faces.size(); ++i) {
			//clock_t threadStart = clock();
			//if (threadStart < Thread4StartTime)
				//Thread4StartTime = threadStart;
			int ID = omp_get_thread_num();
			Wind_F(env->cloth->faces[i], F_Ls[ID]);
			/*
			thread4end = clock();
			if (thread4end - startT > Thread4Time)
				Thread4Time = thread4end - startT;
			if (thread4end - threadStart > Thread41Time)
				Thread41Time = thread4end - threadStart;
			if (thread4end - Thread4StartTime > thread4cost)
				thread4cost = thread4end - Thread4StartTime;
			*/
		}

#pragma omp single
		{
			//Thread5StartTime = clock();
			//clock_t threadStart = clock();
			...
			/*
			thread5end = clock();
			if (thread5end - startT > Thread5Time)
				Thread5Time = thread5end - startT;
			if (thread5end - threadStart > Thread51Time)
				Thread51Time = thread5end - threadStart;
			if (thread5end - Thread5StartTime > thread5cost)
				thread5cost = thread5end - Thread5StartTime;
			*/
		}
...
#pragma omp for collapse(2)
		for (int i = 1; i < env->cloth->length - 1; ++i)
			for (int j = 1; j < env->cloth->width - 1; ++j) {
				//clock_t threadStart = clock();
				//if (threadStart < Thread6StartTime)
					//Thread6StartTime = threadStart;
				int ID = omp_get_thread_num();
				std::string node_idx{ std::to_string(j) + "," + std::to_string(i) };
				std::string node_idx1{ std::to_string(j - 1) + "," + std::to_string(i) };
				std::string node_idx2{ std::to_string(j + 1) + "," + std::to_string(i) };
				std::string node_idx3{ std::to_string(j) + "," + std::to_string(i - 1) };
				std::string node_idx4{ std::to_string(j) + "," + std::to_string(i + 1) };
				...
				if (type == 1) {
					ConSlideFriction(env->cloth->nodes[node_idx], F_E_is_Slides[ID],
						...
						StrhBend_C_Ls[0], StrhBend_NC_Ls[0], StrhBend_C_LEs[0], StrhBend_NC_LEs[0], type);
				}
				else if (type == 2) {
					ConSlideFriction(env->cloth->nodes[node_idx],
						env->cloth->nodes[node_idx1], env->cloth->nodes[node_idx2], env->cloth->nodes[node_idx3], env->cloth->nodes[node_idx4],
						F_E_is_Slides[ID],
						F_E_No_Slide, F_EE_Pos_No_Slide, F_EE_Vel_No_Slide,
						F_Es[ID], F_EE_Poss[ID], F_EE_Vels[ID],
				...
				/*
				thread6end = clock();
				if (thread6end - startT > Thread6Time)
					Thread6Time = thread6end - startT;
				if (thread6end - threadStart > Thread61Time)
					Thread61Time = thread6end - threadStart;
				if (thread6end - Thread6StartTime > thread6cost)
					thread6cost = thread6end - Thread6StartTime;
				*/
			}
...
#pragma omp for
		for (int i = 0; i < env->cloth->shear_segs.size(); ++i) {
			//clock_t threadStart = clock();
			//if (threadStart < Thread7StartTime)
				//Thread7StartTime = threadStart;
			int ID = omp_get_thread_num();
			...
			/*
			thread7end = clock();
			if (thread7end - startT > Thread7Time)
				Thread7Time = thread7end - startT;
			if (thread7end - threadStart > Thread71Time)
				Thread71Time = thread7end - threadStart;
			if (thread7end - Thread7StartTime > thread7cost)
				thread7cost = thread7end - Thread7StartTime;
			*/
		}

	} // End Parallel Region
	/*
	std::cout << "Thread 1 start time: " << Thread1StartTime - startT << "ms; end time:" << thread1end << " ms; cost time: " << thread1cost << " ms;" << " Each step cost time: " << Thread11Time << " ms;" << " Step: " << env->cloth->width * env->cloth->length << std::endl;
	std::cout << "Thread 2 start time: " << Thread2StartTime - startT << "ms; end time:" << thread2end << " ms; cost time: " << thread2cost << " ms;" << " Each step cost time: " << Thread21Time << " ms;" << " Step: " << env->cloth->edges.size() << std::endl;
	std::cout << "Thread 3 start time: " << Thread3StartTime - startT << "ms; end time:" << thread3end << " ms; cost time: " << thread3cost << " ms;" << " Each step cost time: " << Thread31Time << " ms;" << " Step: " << env->cloth->bend_segs.size() << std::endl;
	std::cout << "Thread 4 start time: " << Thread4StartTime - startT << "ms; end time:" << thread4end << " ms; cost time: " << thread4cost << " ms;" << " Each step cost time: " << Thread41Time << " ms;" << " Step: " << env->cloth->faces.size() << std::endl;
	std::cout << "Thread 5 start time: " << Thread5StartTime - startT << "ms; end time:" << thread5end << " ms; cost time: " << thread5cost << " ms;" << " Each step cost time: " << Thread51Time << " ms;" << " NThrd: " << NThrd <<std::endl;
	std::cout << "Thread 6 start time: " << Thread6StartTime - startT << "ms; end time:" << thread6end << " ms; cost time: " << thread6cost << " ms;" << " Each step cost time: " << Thread61Time << " ms;" << " Step: " << (env->cloth->length - 1)*(env->cloth->width - 1) << std::endl;
	std::cout << "Thread 7 start time: " << Thread7StartTime - startT << "ms; end time:" << thread7end << " ms; cost time: " << thread7cost << " ms;" << " Each step cost time: " << Thread71Time << " ms;" << " Step: " << env->cloth->shear_segs.size() << std::endl;
	*/
	for (int t = 1; t < NThrd; ++t) {
...
void PhysicsCloth::ConSlideFriction(Node& n, Node& n1, Node& n2, Node& n3, Node& n4, Tensor& F_E_is_Slide,
	...
	Tensor& StrhBend_C_LE, Tensor& StrhBend_NC_LE)
{
	...
	Tensor uv_minus_uvbar_1{ n1.EPosBar - n1.EPos};
	Tensor uv_minus_uvbar_2{ n2.EPosBar - n2.EPos };
	Tensor uv_minus_uvbar_3{ n3.EPosBar - n3.EPos };
	Tensor uv_minus_uvbar_4{ n4.EPosBar - n4.EPos };
	Tensor uv_minus_uvbar_new = uv_minus_uvbar_1 + uv_minus_uvbar_2 + uv_minus_uvbar_3 + uv_minus_uvbar_4;
	...
	Tensor damping_dir_left{ (n.EPos - n1.EPos) / torch::sqrt(((n.EPos - n1.EPos) * (n.EPos - n1.EPos)).sum()) };
	Tensor damping_left{ -1 * env->cloth->df * (damping_dir_left * (n.EVel - n1.EVel)).sum() * damping_dir_left };
	Tensor damping_dir_right{ (n.EPos - n2.EPos) / torch::sqrt(((n.EPos - n2.EPos) * (n.EPos - n2.EPos)).sum()) };
	Tensor damping_right{ -1 * env->cloth->df * (damping_dir_right * (n.EVel - n2.EVel)).sum() * damping_dir_right };
	Tensor damping_dir_up{ (n.EPos - n3.EPos) / torch::sqrt(((n.EPos - n3.EPos) * (n.EPos - n3.EPos)).sum()) };
	Tensor damping_up{ -1 * env->cloth->df * (damping_dir_up * (n.EVel - n3.EVel)).sum() * damping_dir_up };
	Tensor damping_dir_bottom{ (n.EPos - n4.EPos) / torch::sqrt(((n.EPos - n4.EPos) * (n.EPos - n4.EPos)).sum()) };
	Tensor damping_bottom{ -1 * env->cloth->df * (damping_dir_bottom * (n.EVel - n4.EVel)).sum() * damping_dir_bottom };
	
	Tensor damping_force = damping_left + damping_right + damping_up + damping_bottom;
	Tensor damping_vel_u = torch::pow(damping_dir_left[0], 2) + torch::pow(damping_dir_right[0], 2) + torch::pow(damping_dir_up[0], 2) + torch::pow(damping_dir_bottom[0], 2);
	Tensor damping_vel_v = torch::pow(damping_dir_left[1], 2) + torch::pow(damping_dir_right[1], 2) + torch::pow(damping_dir_up[1], 2) + torch::pow(damping_dir_bottom[1], 2);
	
	Tensor delta_vel_left{ n.EVel - n1.EVel };
	Tensor delta_pos_left{ n.EPos - n1.EPos };
	Tensor diff_u_left = 
		env->cloth->df *
		(((delta_vel_left[0] * torch::pow(delta_pos_left[0], 2) + delta_vel_left[1] * delta_pos_left[0] * delta_pos_left[1]) * 2 * delta_pos_left[0])
		/ torch::pow((torch::pow(delta_pos_left[0], 2) + torch::pow(delta_pos_left[1], 2)), 2)
		- (delta_vel_left[1] * delta_pos_left[1] + 2 * delta_vel_left[0] * delta_pos_left[0])
		/ (torch::pow(delta_pos_left[0],2)+ torch::pow(delta_pos_left[1],2)));
	Tensor diff_v_left =
		env->cloth->df *
		(((delta_vel_left[1] * torch::pow(delta_pos_left[1], 2) + delta_vel_left[0] * delta_pos_left[0] * delta_pos_left[1]) * 2 * delta_pos_left[1])
			/ torch::pow((torch::pow(delta_pos_left[0], 2) + torch::pow(delta_pos_left[1], 2)), 2)
			- (delta_vel_left[0] * delta_pos_left[0] + 2 * delta_vel_left[1] * delta_pos_left[1])
			/ (torch::pow(delta_pos_left[0], 2) + torch::pow(delta_pos_left[1], 2)));

	Tensor delta_vel_right{ n.EVel - n2.EVel };
	Tensor delta_pos_right{ n.EPos - n2.EPos };
	Tensor diff_u_right =
		env->cloth->df *
		(((delta_vel_right[0] * torch::pow(delta_pos_right[0], 2) + delta_vel_right[1] * delta_pos_right[0] * delta_pos_right[1]) * 2 * delta_pos_right[0])
			/ torch::pow((torch::pow(delta_pos_right[0], 2) + torch::pow(delta_pos_right[1], 2)), 2)
			- (delta_vel_right[1] * delta_pos_right[1] + 2 * delta_vel_right[0] * delta_pos_right[0])
			/ (torch::pow(delta_pos_right[0], 2) + torch::pow(delta_pos_right[1], 2)));
	Tensor diff_v_right =
		env->cloth->df *
		(((delta_vel_right[1] * torch::pow(delta_pos_right[1], 2) + delta_vel_right[0] * delta_pos_right[0] * delta_pos_right[1]) * 2 * delta_pos_right[1])
			/ torch::pow((torch::pow(delta_pos_right[0], 2) + torch::pow(delta_pos_right[1], 2)), 2)
			- (delta_vel_right[0] * delta_pos_right[0] + 2 * delta_vel_right[1] * delta_pos_right[1])
			/ (torch::pow(delta_pos_right[0], 2) + torch::pow(delta_pos_right[1], 2)));

	Tensor delta_vel_up{ n.EVel - n3.EVel };
	Tensor delta_pos_up{ n.EPos - n3.EPos };
	Tensor diff_u_up =
		env->cloth->df *
		(((delta_vel_up[0] * torch::pow(delta_pos_up[0], 2) + delta_vel_up[1] * delta_pos_up[0] * delta_pos_up[1]) * 2 * delta_pos_up[0])
			/ torch::pow((torch::pow(delta_pos_up[0], 2) + torch::pow(delta_pos_up[1], 2)), 2)
			- (delta_vel_up[1] * delta_pos_up[1] + 2 * delta_vel_up[0] * delta_pos_up[0])
			/ (torch::pow(delta_pos_up[0], 2) + torch::pow(delta_pos_up[1], 2)));
	Tensor diff_v_up =
		env->cloth->df *
		(((delta_vel_up[1] * torch::pow(delta_pos_up[1], 2) + delta_vel_up[0] * delta_pos_up[0] * delta_pos_up[1]) * 2 * delta_pos_up[1])
			/ torch::pow((torch::pow(delta_pos_up[0], 2) + torch::pow(delta_pos_up[1], 2)), 2)
			- (delta_vel_up[0] * delta_pos_up[0] + 2 * delta_vel_up[1] * delta_pos_up[1])
			/ (torch::pow(delta_pos_up[0], 2) + torch::pow(delta_pos_up[1], 2)));

	Tensor delta_vel_bottom{ n.EVel - n4.EVel };
	Tensor delta_pos_bottom{ n.EPos - n4.EPos };
	Tensor diff_u_bottom =
		env->cloth->df *
		(((delta_vel_bottom[0] * torch::pow(delta_pos_bottom[0], 2) + delta_vel_bottom[1] * delta_pos_bottom[0] * delta_pos_bottom[1]) * 2 * delta_pos_bottom[0])
			/ torch::pow((torch::pow(delta_pos_bottom[0], 2) + torch::pow(delta_pos_bottom[1], 2)), 2)
			- (delta_vel_bottom[1] * delta_pos_bottom[1] + 2 * delta_vel_bottom[0] * delta_pos_bottom[0])
			/ (torch::pow(delta_pos_bottom[0], 2) + torch::pow(delta_pos_bottom[1], 2)));
	Tensor diff_v_bottom =
		env->cloth->df *
		(((delta_vel_bottom[1] * torch::pow(delta_pos_bottom[1], 2) + delta_vel_bottom[0] * delta_pos_bottom[0] * delta_pos_bottom[1]) * 2 * delta_pos_bottom[1])
			/ torch::pow((torch::pow(delta_pos_bottom[0], 2) + torch::pow(delta_pos_bottom[1], 2)), 2)
			- (delta_vel_bottom[0] * delta_pos_bottom[0] + 2 * delta_vel_bottom[1] * delta_pos_bottom[1])
			/ (torch::pow(delta_pos_bottom[0], 2) + torch::pow(delta_pos_bottom[1], 2)));

	Tensor fric_damping_pos_u = diff_u_left + diff_u_right + diff_u_up + diff_u_bottom;
	Tensor fric_damping_pos_v = diff_v_left + diff_v_right + diff_v_up + diff_v_bottom;

	//std::cout << "{" << n.EPos << ", " << n1.EPos << ", " << n2.EPos << ", " << n3.EPos << ", " << n4.EPos << "}" << std::endl;
	//std::cout << std::endl;
	//std::cout << "{" << n.EVel << ", " << n1.EVel << ", " << n2.EVel << ", " << n3.EVel << ", " << n4.EVel << "}" << std::endl;
	//std::cout << std::endl;
	//std::cout<< damping_force <<", "<< -env->cloth->df * n.EVel << std::endl;
	//std::cout << std::endl;
	//std::cout << "{" << damping_dir_left << "," << damping_left << "; " << damping_dir_right << ", " << damping_right << "; " << damping_dir_up<<","<<damping_up << "}" << std::endl;
	//std::cout << std::endl;

	//system("pause");
	Tensor fric_force{ (-0.5 * (env->cloth->kf * uv_minus_uvbar + (env->cloth->kf / 4 * uv_minus_uvbar_new) - torch::tanh(10 * uv_minus_uvbar) * compression * env->cloth->mu) * torch::tanh(10 * (fric_lim - F_E[FE_idx])) + 0.5 * (env->cloth->kf * uv_minus_uvbar + torch::tanh(10 * uv_minus_uvbar) * compression * env->cloth->mu)) + damping_force };
	Tensor fric_u_pos{ (-0.5 * (env->cloth->kf - (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * uv_minus_uvbar[0]),2)) * env->cloth->mu * compression - torch::tanh(10 * uv_minus_uvbar[0]) * env->cloth->mu * compression_EE_u)) * torch::tanh(10 * (fric_lim - F_E_No_Slide[FE_idx])[0]) - (0.5 * (env->cloth->kf * uv_minus_uvbar[0] - torch::tanh(10 * uv_minus_uvbar[0]) * env->cloth->mu * compression)) * (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * (fric_lim - F_E_No_Slide[FE_idx])[0]),2)) * (F_EE_Pos_u - env->cloth->mu * compression_EE_u) - (0.5 * (env->cloth->kf + (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * uv_minus_uvbar[0]),2)) * env->cloth->mu * compression + torch::tanh(10 * uv_minus_uvbar[0]) * env->cloth->mu * compression_EE_u)) + fric_damping_pos_u };
	Tensor fric_v_pos{ (-0.5 * (env->cloth->kf - (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * uv_minus_uvbar[1]),2)) * env->cloth->mu * compression - torch::tanh(10 * uv_minus_uvbar[1]) * env->cloth->mu * compression_EE_v)) * torch::tanh(10 * (fric_lim - F_E_No_Slide[FE_idx])[1]) - (0.5 * (env->cloth->kf * uv_minus_uvbar[1] - torch::tanh(10 * uv_minus_uvbar[1]) * env->cloth->mu * compression)) * (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * (fric_lim - F_E_No_Slide[FE_idx])[1]),2)) * (F_EE_Pos_v - env->cloth->mu * compression_EE_v) - (0.5 * (env->cloth->kf + (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * uv_minus_uvbar[1]),2)) * env->cloth->mu * compression + torch::tanh(10 * uv_minus_uvbar[1]) * env->cloth->mu * compression_EE_v)) + fric_damping_pos_v };

	Tensor fric_u_vel{ (-0.5 * (env->cloth->kf * uv_minus_uvbar[0] - torch::tanh(10 * uv_minus_uvbar[0]) * env->cloth->mu * compression)) * (torch::ones({1}, opts) - torch::pow(torch::tanh((10 * fric_lim - F_E_No_Slide[FE_idx])[0]),2)) * F_EE_Vel_u - env->cloth->df.view({1,1}) * damping_vel_u };
	Tensor fric_v_vel{ (-0.5 * (env->cloth->kf * uv_minus_uvbar[1] - torch::tanh(10 * uv_minus_uvbar[1]) * env->cloth->mu * compression)) * (torch::ones({1}, opts) - torch::pow(torch::tanh((10 * fric_lim - F_E_No_Slide[FE_idx])[1]),2)) * F_EE_Vel_v - env->cloth->df.view({1,1}) * damping_vel_v };
	...
	Tensor& StrhBend_C_LE, Tensor& StrhBend_NC_LE)
{
	...
	// situation 1: Fslide = 0;
	//Tensor fric_force{ torch::tensor({ 0, 0 }, opts).view({ 2,1 }) };

	//situation 2: 0<=Fu<=mu*Fn
	...
	//situation 3: Fu>mu*Fn
	/*
	Tensor fric_force{ -1 * env->cloth->kf * uv_minus_uvbar  - env->cloth->df * n.EVel };
	Tensor fric_u_pos{ -1 * env->cloth->kf
		- (0.5 * (env->cloth->kf * uv_minus_uvbar[0] + env->cloth->mu * compression))
		* (F_EE_Pos_u - env->cloth->mu * compression_EE_u)
		};
	Tensor fric_v_pos{ -1 * env->cloth->kf
		- (0.5 * (env->cloth->kf * uv_minus_uvbar[1] + env->cloth->mu * compression))
		* (F_EE_Pos_v - env->cloth->mu * compression_EE_v)};

	Tensor fric_u_vel{ (-0.5 * (env->cloth->kf * uv_minus_uvbar[0] + env->cloth->mu * compression)) * F_EE_Vel_u - env->cloth->df.view({1,1}) };
	Tensor fric_v_vel{ (-0.5 * (env->cloth->kf * uv_minus_uvbar[1] + env->cloth->mu * compression)) * F_EE_Vel_v - env->cloth->df.view({1,1}) };
	*/

	// our model
	//Tensor K_delta_U{ torch::tanh(10 * uv_minus_uvbar) };
	//Tensor K_mu_Fn{ torch::tanh(10 * (fric_lim - F_E[FE_idx])) };

	//Tensor K_delta_U_u{ torch::tanh(10 * uv_minus_uvbar[0]) };
	//Tensor K_mu_Fn_u{ torch::tanh((10 * fric_lim - F_E_No_Slide[FE_idx])[0]) };

	//Tensor K_delta_U_v{ torch::tanh(10 * uv_minus_uvbar[1]) };
	//Tensor K_mu_Fn_v{ torch::tanh((10 * fric_lim - F_E_No_Slide[FE_idx])[1]) };


	// Coulomb model
	//Tensor K_delta_U{ torch::ones({ 2, 1 }, opts) },	K_mu_Fn{ -1 * torch::ones({ 2, 1 }, opts) };
	//Tensor K_delta_U_u{ torch::ones({ 1, 1 }, opts) },	K_mu_Fn_u{ -1 * torch::ones({ 1, 1 }, opts) };
	//Tensor K_delta_U_v{ torch::ones({ 1, 1 }, opts) },	K_mu_Fn_v{ -1 * torch::ones({ 1, 1 }, opts) };

	// Stribeck effect
	//Tensor K_delta_U{ -1 * torch::ones({ 2, 1 }, opts) }, K_mu_Fn{ torch::ones({ 2, 1 }, opts) };
	//Tensor K_delta_U_u{ -1 * torch::ones({ 1, 1 }, opts) }, K_mu_Fn_u{ torch::ones({ 1, 1 }, opts) };
	//Tensor K_delta_U_v{ -1 * torch::ones({ 1, 1 }, opts) }, K_mu_Fn_v{ torch::ones({ 1, 1 }, opts) };

	//Tensor fric_force{ (-0.5 * (env->cloth->kf * uv_minus_uvbar - K_delta_U * compression * env->cloth->mu) * K_mu_Fn + 0.5 * (env->cloth->kf * uv_minus_uvbar + K_delta_U * compression * env->cloth->mu)) - env->cloth->df * n.EVel };
	//Tensor fric_u_pos{ (-0.5 * (env->cloth->kf - (torch::ones({1,1}, opts) - torch::pow(K_delta_U_u,2)) * env->cloth->mu * compression - K_delta_U_u * env->cloth->mu * compression_EE_u)) * K_mu_Fn_u - (0.5 * (env->cloth->kf * uv_minus_uvbar[0] - K_delta_U_u * env->cloth->mu * compression)) * (torch::ones({1,1}, opts) - torch::pow(K_mu_Fn_u,2)) * (F_EE_Pos_u - env->cloth->mu * compression_EE_u) - (0.5 * (env->cloth->kf + (torch::ones({1,1}, opts) - torch::pow(K_delta_U_u,2)) * env->cloth->mu * compression + K_delta_U_u * env->cloth->mu * compression_EE_u)) };
	//Tensor fric_v_pos{ (-0.5 * (env->cloth->kf - (torch::ones({1,1}, opts) - torch::pow(K_delta_U_v,2)) * env->cloth->mu * compression - K_delta_U_v * env->cloth->mu * compression_EE_v)) * K_mu_Fn_v - (0.5 * (env->cloth->kf * uv_minus_uvbar[1] - K_delta_U_v * env->cloth->mu * compression)) * (torch::ones({1,1}, opts) - torch::pow(K_mu_Fn_v,2)) * (F_EE_Pos_v - env->cloth->mu * compression_EE_v) - (0.5 * (env->cloth->kf + (torch::ones({1,1}, opts) - torch::pow(K_delta_U_v,2)) * env->cloth->mu * compression + K_delta_U_v * env->cloth->mu * compression_EE_v)) };

	//Tensor fric_u_vel{ (-0.5 * (env->cloth->kf * uv_minus_uvbar[0] - K_delta_U_u * env->cloth->mu * compression)) * (torch::ones({1}, opts) - torch::pow(K_mu_Fn_u,2)) * F_EE_Vel_u - env->cloth->df.view({1,1}) };
	//Tensor fric_v_vel{ (-0.5 * (env->cloth->kf * uv_minus_uvbar[1] - K_delta_U_v * env->cloth->mu * compression)) * (torch::ones({1}, opts) - torch::pow(K_mu_Fn_v,2)) * F_EE_Vel_v - env->cloth->df.view({1,1}) };
...
void PhysicsCloth::ConSlideFriction(Node& n, Tensor& F_E_is_Slide,
	...
	Tensor& F_E, Tensor& F_EE_Pos, Tensor& F_EE_Vel,
	Tensor& StrhBend_C_L, Tensor& StrhBend_NC_L,
	Tensor& StrhBend_C_LE, Tensor& StrhBend_NC_LE, int type)
{
	int FL_idx{ n.idx_u + (n.idx_v * env->cloth->width) };
	int FE_idx{ (n.idx_u - 1 + (env->cloth->width - 2) * (n.idx_v - 1)) };
	...
	*/
	...
	Tensor fric_force{ (-0.5 * (env->cloth->kf * torch::pow(uv_minus_uvbar,2) - torch::tanh(10 * uv_minus_uvbar) * compression * env->cloth->mu) * torch::tanh(10 * (fric_lim - F_E[FE_idx])) + 0.5 * (env->cloth->kf * torch::pow(uv_minus_uvbar,2) + torch::tanh(10 * uv_minus_uvbar) * compression * env->cloth->mu)) - env->cloth->df * n.EVel };
	Tensor fric_u_pos{ (-0.5 * (env->cloth->kf * uv_minus_uvbar[0] - (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * uv_minus_uvbar[0]),2)) * env->cloth->mu * compression - torch::tanh(10 * uv_minus_uvbar[0]) * env->cloth->mu * compression_EE_u)) * torch::tanh(10 * (fric_lim - F_E_No_Slide[FE_idx])[0]) - (0.5 * (env->cloth->kf * torch::pow(uv_minus_uvbar[0],2) - torch::tanh(10 * uv_minus_uvbar[0]) * env->cloth->mu * compression)) * (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * (fric_lim - F_E_No_Slide[FE_idx])[0]),2)) * (F_EE_Pos_u - env->cloth->mu * compression_EE_u) - (0.5 * (env->cloth->kf * uv_minus_uvbar[0] + (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * uv_minus_uvbar[0]),2)) * env->cloth->mu * compression + torch::tanh(10 * uv_minus_uvbar[0]) * env->cloth->mu * compression_EE_u)) };
	Tensor fric_v_pos{ (-0.5 * (env->cloth->kf * uv_minus_uvbar[1] - (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * uv_minus_uvbar[1]),2)) * env->cloth->mu * compression - torch::tanh(10 * uv_minus_uvbar[1]) * env->cloth->mu * compression_EE_v)) * torch::tanh(10 * (fric_lim - F_E_No_Slide[FE_idx])[1]) - (0.5 * (env->cloth->kf * torch::pow(uv_minus_uvbar[1],2) - torch::tanh(10 * uv_minus_uvbar[1]) * env->cloth->mu * compression)) * (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * (fric_lim - F_E_No_Slide[FE_idx])[1]),2)) * (F_EE_Pos_v - env->cloth->mu * compression_EE_v) - (0.5 * (env->cloth->kf * uv_minus_uvbar[1] + (torch::ones({1,1}, opts) - torch::pow(torch::tanh(10 * uv_minus_uvbar[1]),2)) * env->cloth->mu * compression + torch::tanh(10 * uv_minus_uvbar[1]) * env->cloth->mu * compression_EE_v)) };

	Tensor fric_u_vel{ (-0.5 * (env->cloth->kf * torch::pow(uv_minus_uvbar[0],2) - torch::tanh(10 * uv_minus_uvbar[0]) * env->cloth->mu * compression)) * (torch::ones({1}, opts) - torch::pow(torch::tanh((10 * fric_lim - F_E_No_Slide[FE_idx])[0]),2)) * F_EE_Vel_u - env->cloth->df.view({1,1}) };
	Tensor fric_v_vel{ (-0.5 * (env->cloth->kf * torch::pow(uv_minus_uvbar[1],2) - torch::tanh(10 * uv_minus_uvbar[1]) * env->cloth->mu * compression)) * (torch::ones({1}, opts) - torch::pow(torch::tanh((10 * fric_lim - F_E_No_Slide[FE_idx])[1]),2)) * F_EE_Vel_v - env->cloth->df.view({1,1}) };
	//situation 3: Fu>mu*Fn
	/*
	Tensor fric_force{ -1 * env->cloth->kf * uv_minus_uvbar  - env->cloth->df * n.EVel };
	Tensor fric_u_pos{ -1 * env->cloth->kf
	...
	*/
	...
	// our model
	if (type == 1) {
		K_delta_U = { torch::tanh(10 * uv_minus_uvbar) };
		K_mu_Fn = { torch::tanh(10 * (fric_lim - F_E[FE_idx])) };

		K_delta_U_u = { torch::tanh(10 * uv_minus_uvbar[0]) };
		K_mu_Fn_u = { torch::tanh((10 * fric_lim - F_E_No_Slide[FE_idx])[0]) };

		K_delta_U_v = { torch::tanh(10 * uv_minus_uvbar[1]) };
		K_mu_Fn_v = { torch::tanh((10 * fric_lim - F_E_No_Slide[FE_idx])[1]) };
	}
	// Coulomb model
	else if (type == 2) {
		K_delta_U = { torch::ones({ 2, 1 }, opts) }, K_mu_Fn = { -1 * torch::ones({ 2, 1 }, opts) };
		K_delta_U_u = { torch::ones({ 1, 1 }, opts) }, K_mu_Fn_u = { -1 * torch::ones({ 1, 1 }, opts) };
		K_delta_U_v = { torch::ones({ 1, 1 }, opts) }, K_mu_Fn_v = { -1 * torch::ones({ 1, 1 }, opts) };
	}
	// Stribeck effect
	else if (type == 3) {
		K_delta_U = { -1 * torch::ones({ 2, 1 }, opts) }, K_mu_Fn = { torch::ones({ 2, 1 }, opts) };
		K_delta_U_u = { -1 * torch::ones({ 1, 1 }, opts) }, K_mu_Fn_u = { torch::ones({ 1, 1 }, opts) };
		K_delta_U_v = { -1 * torch::ones({ 1, 1 }, opts) }, K_mu_Fn_v = { torch::ones({ 1, 1 }, opts) };
	}
	// Coulomb model
	//Tensor K_delta_U{ torch::ones({ 2, 1 }, opts) }, K_mu_Fn{ -1 * torch::ones({ 2, 1 }, opts) };
	//Tensor K_delta_U_u{ torch::ones({ 1, 1 }, opts) }, K_mu_Fn_u{ -1 * torch::ones({ 1, 1 }, opts) };
