...
void BVHTree::Construct()
{
	...
	for (unsigned int i = 0; i < num_tri; i++) {
		tri_idx++;
		Tensor x;
		x = _mdl->faces[i].v[0]->node->x; vec3f p1 = dat2vec3(x.data_ptr<double>());
		x = _mdl->faces[i].v[1]->node->x; vec3f p2 = dat2vec3(x.data_ptr<double>());
		x = _mdl->faces[i].v[2]->node->x; vec3f p3 = dat2vec3(x.data_ptr<double>());
		x = _mdl->faces[i].v[0]->node->x_prev; vec3f pp1 = dat2vec3(x.data_ptr<double>());
		x = _mdl->faces[i].v[1]->node->x_prev; vec3f pp2 = dat2vec3(x.data_ptr<double>());
		x = _mdl->faces[i].v[2]->node->x_prev; vec3f pp3 = dat2vec3(x.data_ptr<double>());
		if (ccd) {
...
kDOP18 node_box(const MeshNode* node, bool ccd) {
	...
	kDOP18 box(dat2vec3(x.data_ptr<double>()));
