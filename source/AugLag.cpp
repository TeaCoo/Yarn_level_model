...
static void auglag_value_and_grad(const real_1d_array& x, double& value, real_1d_array& grad, void* ptr = nullptr);

static void multiplier_update(const real_1d_array& x);

std::vector<double> augmented_lagrangian_method(NLConOpt& problem, const Tensor& thickness, OptOptions opt) {
    ::problem = &problem;
    ::lambda = std::vector<double>(::problem->ncon, 0);
    ::mu = 1e3;
    ::thick = thickness;
    real_1d_array x;
...
static void auglag_value_and_grad(const real_1d_array& x, double& value,
    ...
    static std::vector<double>* grads = new std::vector<double>[nthreads];
...
#pragma omp parallel for
    for (int j = 0; j < ::problem->ncon; j++) {
        ...
        int sign;
        double gj = ::problem->constraint(&x[0], j, sign, thick);
        double cj = clamp_violation(gj + ::lambda[j] / ::mu, sign);
        if (cj != 0) {
            values[t] += ::mu / 2 * pow(cj,2);
