...
void TestTrain() {
	...
	// Path of training data
	std::string data_path = "./data/Wind_Yarn_1_2_Plain_Size17.h5";
	// Set file saving training result
	std::string save_path{ "./train/HangHeter.h5" };
	// Go into the following function to set parameters' learning rate
	train_heter_full(save_path, data_path, epochs, 1, 1, frame_num + 1, steps, false, false, wordy);
...
int main() {
	...
	//Tensor damping_left{ -1 * df * torch::dot(torch::norm(EPos - EPos1),EVel - EVel1) * torch::norm(EPos - EPos1) };
	//std::cout << torch::dot(torch::norm(EPos - EPos1), EVel - EVel1) << std::endl;
	SimHomo();
	SimHomo();
	SimHomo();
	SimHomo();
	...
	std::cout << "end time: " << end_time << " ms; " << "cost: " << end_time - start_time << " ms" << std::endl;
	*/
	 //TrainHeterFew();
