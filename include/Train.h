...
void train(std::string save_path, std::string data_path, int epoch, int iter, int batch, int sample, int steps, bool random) {
	...
	H5File file{save_path.data(), H5F_ACC_TRUNC};
	...
	std::cout << "load property start time: " << start_time << " ms; ";
	std::cout << "load property end time: " << end_time << " ms; " << "cost: " << end_time - start_time << " ms" << std::endl;
	...
	for (int epc = 0; epc < epoch; ++epc) {
		...
		for (int i = 0; i < iteration; ++i) {
			...
			for (int bch = 0; bch < batch; ++bch) {
				...
				end_time = clock();
				std::cout << "load data start time: " << start_time << " ms; ";
				std::cout << "load data end time: " << end_time << " ms; " << "cost: " << end_time - start_time << " ms" << std::endl;
				...
				for (int smp = 0; smp < sample - 1; ++smp) {
					...
					end_time = clock();
					std::cout << "forward start time: " << start_time << " ms; ";
					std::cout << "forward end time: " << end_time << " ms; " << "cost: " << end_time - start_time << " ms" << std::endl;
			...
			end_time = clock();
			std::cout << "backward start time: " << start_time << " ms; ";
			std::cout << "backward end time: " << end_time << " ms; " << "cost: " << end_time - start_time << " ms" << std::endl;
			...
			std::cout << "<" << constrained_para<<">" << std::endl;

			start_time = clock();
			SaveTrainProc(file, curr_step, train_steps, loss, constrained_para); // save loss and parameters
			end_time = clock();
			std::cout << "save start time: " << start_time << " ms; ";
			std::cout << "save end time: " << end_time << " ms; " << "cost: " << end_time - start_time << " ms" << std::endl;
		}
	}
	file.close();	
}