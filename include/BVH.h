...
inline Tensor mid_xyz(const Tensor& p1, const Tensor& p2, const Tensor& p3) {
	Tensor max_pos = torch::max(torch::max(p1, p2), p3);
	Tensor min_pos = torch::min(torch::min(p1, p2), p3);
	return 0.5 * (max_pos + min_pos);
}
...
public:
	...
	{
		vec3f p = dat2vec3(p0.data_ptr<double>());
		return *this += p;
	}
...
struct BVHNode
{
	...
	BVHNode();
	BVHNode(BVHNode* _parent, MeshFace* _face, kDOP18* face_boxes, vec3f* face_center);
	BVHNode(BVHNode* _parent, MeshFace** face_buffer, int lst_num, kDOP18* face_boxes, vec3f* face_center);
...
struct BVHTree {
	...
	Mesh* _mdl;
	BVHNode* _root;
	MeshFace** face_buffer;

	BVHTree(bool ccd, Mesh& mesh);
	~BVHTree();

	bool ccd; // Is continuous collision detection
	...
	kDOP18 box();
	inline BVHNode* getRoot() { return _root; };

	friend class DeoformBVHNode;
};
