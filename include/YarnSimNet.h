...
struct YarnSimNet : torch::nn::Module {
	...
	ClothState forward(ClothState cloth_state, bool& is_success) {
		...
		std::cout << "set_ClothState start time: " << start_time << " ms; ";
		std::cout << "set_ClothState end time: " << end_time << " ms; " << "cost: " << end_time - start_time << " ms" << std::endl;

		start_time = clock();
		is_success = SimStep(cloth, phy_sim, h);
		end_time = clock();
		std::cout << "SimStep start time: " << start_time << " ms; ";
		std::cout << "SimStep end time: " << end_time << " ms; " << "cost: " << end_time - start_time << " ms" << std::endl;
