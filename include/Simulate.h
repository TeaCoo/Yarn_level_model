...
void SimHomo() {
	...
	std::shared_ptr<H5File> file = std::make_shared<H5File>(h5path.data(), H5F_ACC_TRUNC);
	...
	for (int i = 0; i < steps; ++i) {
		...
		std::string save_path = "./objs/new/" + objname + ".obj"; // Path of .obj files
	...
	std::cout << threadEnd - threadStart << std::endl;
	file->close();
}

void SimHomo_1(int type = 1) {
	// Parallel Simulation Pure Woven Cloth
	...
	for (int i = 0; i < steps; ++i) {
		...
		if(type == 1)
			save_path = "./objs/5x5/our/" + objname + ".obj"; // Path of .obj files
		else if(type == 2)
			save_path = "./objs/5x5/stribeck/" + objname + ".obj"; // Path of .obj files
